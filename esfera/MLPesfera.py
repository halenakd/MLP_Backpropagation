...
class MLP:
    ...
    def backward_pass(self, inputs, targets):
        ...
        for j in range(len(inputs)):
            ...
            e = targets[j] - activations[-1] # Erro da última camada

            s_L = -2 * self.purelin_derivative(activations[-1]) * e    # Para a última camada
...
# ex.: 2, 2, 1
layers = [2, 10, 3]       # Quantidade de neurônios de cada camada => define o número de camadas ocultas
...
angles, coords = importar_dados(r'esfera\coords1.csv')         # Importa os ângulos e senos que serão usados na predição e cálculo do erro e gráfico da rede
inputs, targets = importar_dados(r'esfera\padroes.csv')     # Importa os padrões que serão utilizados no treinamento
...
for pattern in range(len(angles)):
    ...
    error = np.sqrt(np.sum((coords[pattern] - prediction)**2))
    errors.append(error)

    #print("Input:", angles[pattern], " Prediction:", prediction, " Target:", coords[pattern], " Error:", error)

    inputsplot.append(angles[pattern])
    predictionsplot.append(prediction)
    errors.append(error)

print("Min_error:", np.min(errors))
print("Max_error:", np.max(errors))

# CÁLCULO DO ERRO QUADRÁTICO MÉDIO

total_error = np.sum(errors)
rmse = np.sqrt(total_error / len(errors))
...
predictionsplot = np.array(predictionsplot)

fig = plt.figure(figsize=(12, 10))
ax = fig.add_subplot(111, projection='3d')

ax.scatter(coords[:, 0], coords[:, 1], coords[:, 2], color='red', s=50, label='Pontos Coordenados')
ax.scatter(predictionsplot[:, 0], predictionsplot[:, 1], predictionsplot[:, 2], color='blue', s=50, label='Pontos Gerados pela Rede')

ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
ax.set_title('Gráfico 3D de Pontos Coordenados e Pontos Gerados pela Rede')
